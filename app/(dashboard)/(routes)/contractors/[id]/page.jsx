...
const ContractorPage = ({params}) => {
    ...
    const {data, error, isLoading} = useSWR(
        `${process.env.NEXT_PUBLIC_api_url}/api/contractor/${id}`,
        fetcher
    );
    ...
    // const numTrucks = contractor.truckList.length;
    // const numDrivers = contractor.driverList.length;
    ...
    const handleReturnTruckFromMaintenance = async (truck) => {
        const result = await PUTAPI(`/api/contractor/${contractor.code}/returnFromMaintenance`,truck)
        setContractor(result)
    }
    ...
    const handleDriverCreated = (contractor) => {
        setContractor(contractor)
    }
    const handleTruckCreated = (contractor) => {
        setContractor(contractor)
    }
            ...
            <div>
              ...
              <div className="flex justify-between shadow-md p-2">
                ...
                <div className="text-right ">{numDrivers || "..........."}</div>
                      ...
                      {contractor.truckList.length} Trucks
                    </span>
                                    </div>
                                    <div className="text-md flex justify-center items-center">
                                        <UserCheck2 className="mr-2 text-sky-500"/>
                                        <span className="font-bold">
                      {contractor.driverList.length} Drivers
                    </span>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="px-4">
                    {showTrucks && (
                        <TrucksData
                            ...
                            onReturnTruck={handleReturnTruckFromMaintenance}
                        />
                    )}
                    {!showTrucks && (
                        <DriversData
